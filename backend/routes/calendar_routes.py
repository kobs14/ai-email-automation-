...
def list_events():
    ...
    if start and end:
        try:
            ...
            raise APIError("Invalid date format. Use ISO 8601.", status_code=400) from err

        results = db.execute_query(
            calendar_queries.GET_CALENDAR_EVENTS_IN_RANGE, params=(start_dt, end_dt), fetch="all"
        )
    else:
        results = db.execute_query(calendar_queries.GET_UPCOMING_CALENDAR_EVENTS, params=(100,), fetch="all")
...
def upcoming_events():
    ...
    results = db.execute_query(calendar_queries.GET_UPCOMING_CALENDAR_EVENTS, params=(limit,), fetch="all")
    items = [_serialize_event(dict(r)) for r in results] if results else []
    return jsonify({"items": items}), 200
...
def get_event(event_id: int):
    ...
    result = db.execute_query(calendar_queries.GET_CALENDAR_EVENT_BY_ID, params=(event_id,), fetch="one")
    if not result:
