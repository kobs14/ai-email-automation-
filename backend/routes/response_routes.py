...
def list_responses():
    ...
    count_result = db.execute_query(
        email_queries.COUNT_RESPONSES_FILTERED, params=(status_filter, status_filter), fetch="one"
    )
    total = count_result["count"] if count_result else 0

    results = db.execute_query(
        email_queries.GET_RESPONSES_PAGINATED, params=(status_filter, status_filter, per_page, offset), fetch="all"
    )
...
def update_response_content(response_id: int):
    ...
    if response["status"] not in ("draft", "rejected"):
        raise APIError("Only draft or rejected responses can be edited", status_code=400)
...
def approve_response(response_id: int):
    ...
    if response["status"] != "draft":
        raise APIError(f"Cannot approve response with status '{response['status']}'", status_code=400)
...
def reject_response(response_id: int):
    ...
    if response["status"] != "draft":
        raise APIError(f"Cannot reject response with status '{response['status']}'", status_code=400)
...
def retry_response(response_id: int):
    ...
    if response["status"] != "failed":
        raise APIError("Only failed responses can be retried", status_code=400)
