...
def cmd_approve(args):
    ...
    if success:
        ...
        print("\nTo send immediately:")
        print("  from services.tasks.email_tasks import send_single_response_task")
...
def cmd_edit(args):
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
        ...
        f.write("# Save and close to continue\n")
        f.write("#\n")
    ...
    try:
        ...
        edited_content = "".join(line for line in lines if not line.startswith("#")).strip()
...
def cmd_failed(args):
    ...
    for resp in failed:
        ...
        print(
            f"{resp['id']:>5} | {truncate(resp['from_address'], 25):^25} | {resp.get('send_attempts', 0):^8} | {error}"
        )
...
Examples:
    ...
    approve_parser.add_argument("--approver", "-a", help="Approver name (default: current user)")
    ...
    edit_parser.add_argument("--approver", "-a", help="Approver name (default: current user)")
    edit_parser.add_argument("--no-approve", action="store_true", help="Do not prompt to approve after editing")
    edit_parser.set_defaults(func=cmd_edit)

    # failed command
    failed_parser = subparsers.add_parser("failed", help="List failed sends")
    failed_parser.add_argument("--limit", "-l", type=int, default=50, help="Maximum number of results")
    ...
    retry_parser.add_argument(
        "--immediate", "-i", action="store_true", help="Send immediately instead of waiting for batch"
    )
