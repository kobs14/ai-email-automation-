...
class EventBuilder:
    ...
    def build_from_entities(
        ...
        """
        # Extract date/time
        date_str = entities.get("requested_date") or entities.get("preferred_date")
        time_str = entities.get("requested_time") or entities.get("preferred_time")
        ...
        if parsed is None:
            logger.warning(f"Cannot build event: unable to parse date/time (date='{date_str}', time='{time_str}')")
            return None
        ...
        logger.info(f"Built calendar event: '{title}' at {start_dt.isoformat()}")
        return event_body
...
class ConflictChecker:
    ...
    def check_conflicts(
        ...
        if conflicts:
            logger.info(
                f"Found {len(conflicts)} conflicting events for {start_time.isoformat()} - {end_time.isoformat()}"
            )
