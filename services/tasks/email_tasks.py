...
def fetch_emails_task(self, max_results: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Email fetch complete: {len(fetched_emails)} processed, {len(new_emails)} new, {len(errors)} errors"
        )
...
def process_pending_emails_task(self, batch_size: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        for email_record in pending_emails:
            ...
            try:
                # Process with Claude
                result = _process_email_with_claude(email_record, email_repo, entity_repo, response_repo)
                results.append(
                    {
                        "email_id": email_id,
                        "status": "success",
...
def process_single_email_task(self, email_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Processing: {email_record.get('from_address')} | Subject: {email_record.get('subject', '')[:50]}")
...
def _process_email_with_claude(email_record: Dict[str, Any], email_repo, entity_repo, response_repo) -> Dict[str, Any]:
    """
    ...
    claude_result = process_email_with_claude(
        body=body, from_address=from_address, subject=subject, generate_response_flag=True
    )
    ...
    response_id = None
    if response_text:
        response_id = response_repo.create_response(email_id=email_id, draft_content=response_text, status="draft")
...
def send_approved_responses_task(self, batch_size: int = 10, max_attempts: int = 3) -> Dict[str, Any]:
    """
    ...
    logger.info(f"Starting send approved responses task (batch_size={batch_size}, max_attempts={max_attempts})")
    ...
    try:
        ...
        responses = response_repo.get_approved_for_sending(max_attempts=max_attempts, limit=batch_size)
        ...
        for resp in responses:
            response_id = resp["id"]
            try:
                result = _send_single_response(resp, sender, response_repo, email_repo, max_attempts)
        ...
        logger.info(f"Send task complete: {sent_count} sent, {failed_count} failed, {len(results)} total")
...
def send_single_response_task(self, response_id: int, max_attempts: int = 3) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if resp["status"] != "approved":
            logger.warning(f"Response {response_id} is not approved (status={resp['status']})")
            return {
                "status": "error",
                "response_id": response_id,
...
def _send_single_response(resp: Dict[str, Any], sender, response_repo, email_repo, max_attempts: int) -> Dict[str, Any]:
    """
    ...
    logger.info(f"Sending response {response_id} to {to_address} | Thread: {thread_id or 'new'}")

    # Send the email
    if thread_id and message_id:
        send_result = sender.send_reply(
            to=to_address, subject=subject, body=body, thread_id=thread_id, in_reply_to=message_id
        )
    else:
        send_result = sender.send_new_email(to=to_address, subject=subject, body=body)

    if send_result.success:
        # Mark response as sent
        response_repo.mark_sent_with_message_id(response_id, send_result.message_id)
        ...
        try:
            ...
            if app_settings.calendar.is_configured():
                ...
                logger.info(f"Queued calendar event creation for response {response_id}")
        except Exception as e:
            logger.warning(f"Failed to queue calendar event task for response {response_id}: {e}")
