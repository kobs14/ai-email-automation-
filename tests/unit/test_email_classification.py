...
try:
    from services.claude.classifier import classify_email, VALID_INTENTS
...
class TestClassifyEmailMocked:
    ...
    def test_classifies_quote_request(self):
        ...
        with patch('services.claude.classifier.get_claude_client') as mock_get_client:
            ...
            mock_client = Mock()
            mock_response = Mock()
            mock_response.content = [Mock(text='{"intent": "quote_request", "confidence": 0.95, "reasoning": "Customer asking for price"}')]
            mock_client.messages.create.return_value = mock_response
            mock_get_client.return_value = mock_client

            result = classify_email(
                body="I need a quote for cleaning my house",
    ...
    def test_classifies_booking_request(self):
        ...
        with patch('services.claude.classifier.get_claude_client') as mock_get_client:
            mock_client = Mock()
            mock_response = Mock()
            mock_response.content = [Mock(text='{"intent": "booking_request", "confidence": 0.90, "reasoning": "Customer wants to book"}')]
            mock_client.messages.create.return_value = mock_response
            mock_get_client.return_value = mock_client

            result = classify_email(
                body="I want to book a cleaning for next week",
                subject="Booking",
                from_address="test@example.com"
            )
    ...
    def test_classifies_rescheduling(self):
        ...
        with patch('services.claude.classifier.get_claude_client') as mock_get_client:
            mock_client = Mock()
            mock_response = Mock()
            mock_response.content = [Mock(text='{"intent": "rescheduling", "confidence": 0.85, "reasoning": "Customer needs to reschedule"}')]
            mock_client.messages.create.return_value = mock_response
            mock_get_client.return_value = mock_client
    ...
    def test_classifies_complaint(self):
        ...
        with patch('services.claude.classifier.get_claude_client') as mock_get_client:
            mock_client = Mock()
            mock_response = Mock()
            mock_response.content = [Mock(text='{"intent": "complaint", "confidence": 0.88, "reasoning": "Customer unhappy"}')]
            mock_client.messages.create.return_value = mock_response
            mock_get_client.return_value = mock_client

            result = classify_email(
                body="I'm unhappy with the service",
                subject="Complaint",
                from_address="test@example.com"
            )
    ...
    def test_classifies_general_inquiry(self):
        ...
        with patch('services.claude.classifier.get_claude_client') as mock_get_client:
            mock_client = Mock()
            mock_response = Mock()
            mock_response.content = [Mock(text='{"intent": "general_inquiry", "confidence": 0.75, "reasoning": "General question"}')]
            mock_client.messages.create.return_value = mock_response
            mock_get_client.return_value = mock_client
