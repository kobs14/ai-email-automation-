...
from services.email_sender import EmailSender, SendResult, ErrorType
...
class TestEmailSender:
    ...
    def test_send_reply_success(self, mock_auth_class, mock_client_class):
        ...
        # Setup mock
        mock_client = Mock()
        mock_client.send_reply.return_value = {
            'id': 'reply_msg_123',
...
class TestSendResponseEmail:
    ...
    def test_sends_reply_when_thread_info_available(self, mock_sender_class):
        ...
        mock_sender = Mock()
        mock_sender.send_reply.return_value = SendResult(
            success=True,
            message_id="msg123",
            thread_id="thread456"
        )
